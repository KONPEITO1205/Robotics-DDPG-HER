...
class actor(nn.Module):
    def __init__(self, env_params, params):
        ...
        self.conv1 = nn.Conv2d(params.input_channel, 24, kernel_size=8, stride=4)
        ...
        self.fc1 = nn.Linear(1600+12, 1024)
    ...
    def forward(self, x, g_coord):
        ...
        # print(g.shape)
        x = F.relu(self.fc1(torch.cat([x.view(-1, 1600)[0], g[0]]).unsqueeze(0)))
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
...
class critic(nn.Module):
    ...
    def forward(self, x, actions, g_coord):
        ...
        x = F.relu(self.fc1(x.view(-1, 1600)[0]))
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        # print(x.shape)  # -> [256]
